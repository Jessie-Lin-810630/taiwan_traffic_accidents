...
| 4 | `read_sql` 與 `exec_sql_linebyline` 的 `finally` **比照 `upsert_to_table` 加守衛**；`conn` / `cursor` 初始化為 `None` 並在 `try` 內綁定 | repo 內已有一致的寫法，照抄使三處風格統一。抽成共用的 context manager 屬候選 1（重複樣板）的範疇，病因不同 |
...
- 子決策 4 讓 `finally` 的守衛樣板在 repo 內出現第三、四次。
  這是刻意接受的重複 —— 消除它需要設計新介面，屬候選 1。
