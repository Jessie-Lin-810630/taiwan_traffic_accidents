...
def main():
    ...
    with col_left:
        st.markdown(
            '<div class="section-title" style="margin-top:0px;">切換分析視角</div>',
            unsafe_allow_html=True,
        )
        mode = st.radio(
            "切換分析視角",
        ...
        st.markdown(
            '<div class="section-title">🗺️ 分析區域與縣市篩選</div>',
            unsafe_allow_html=True,
        )
        ...
        with st.container(border=True):
            ...
            sel_year_str = st.selectbox(
                "年份", ["全部年份"] + [str(y) for y in years_available]
            )
        ...
        display_area_name = (
            sel_city
            if sel_city != "全部縣市"
            else (sel_region if sel_region != "全部區域" else "全台")
        )
        entity_label = (
            "選擇的縣市" if target_entity_col == "city" else "左側選單選擇的夜市"
        )

        df_time_base = df_raw.copy()
        if sel_q != "全年":
            df_time_base = df_time_base[
                df_time_base["Quarter"] == int(sel_q.split()[1])
            ]
        if sel_m != "全部":
            df_time_base = df_time_base[df_time_base["Month"] == int(sel_m.split()[0])]
        if sel_w != "全部":
            df_time_base = df_time_base[df_time_base["Weekday"] == week_map[sel_w]]
        if "白天" in heat_mode:
            df_time_base = df_time_base[
                (df_time_base["Hour"] >= 6) & (df_time_base["Hour"] < 18)
            ]
        elif "夜間" in heat_mode:
            df_time_base = df_time_base[
                (df_time_base["Hour"] >= 18) | (df_time_base["Hour"] < 6)
            ]

        df_area_base = df_time_base.copy()
        if sel_region != "全部區域":
            df_area_base = df_area_base[df_area_base["region"] == sel_region]
        if sel_city != "全部縣市":
            df_area_base = df_area_base[df_area_base["city"] == sel_city]
    ...
    with col_right:
        st.markdown(
            f"<div style='font-size:17px; font-weight:bold; color:#1f2937; margin-top:5px; margin-bottom:12px;'>🚦 {display_area_name} 安全基準線與現況排名</div>",
            unsafe_allow_html=True,
        )
        ...
        if sel_city != "全部縣市":
            kpi_benchmark = (
                df_nat_current.groupby("city")[agg_col].agg(agg_func).mean()
                if not df_nat_current.empty
                else 0
            )
            sub_benchmark = (
                df_nat_current.groupby("city")[sub_col].agg(sub_func).mean()
                if not df_nat_current.empty
                else 0
            )
        elif sel_region != "全部區域":
            kpi_benchmark = (
                df_nat_current.groupby("region")[agg_col].agg(agg_func).mean()
                if not df_nat_current.empty
                else 0
            )
            sub_benchmark = (
                df_nat_current.groupby("region")[sub_col].agg(sub_func).mean()
                if not df_nat_current.empty
                else 0
            )
        else:
        ...
        if sel_year_str != "全部年份":
            ...
            if not df_prev.empty:
                ...
                if prev_val > 0:
                    ...
                    yoy_text = (
                        "↑ 惡化" if yoy_val > 0 else "↓ 進步" if yoy_val < 0 else "持平"
                    )
                    yoy_class = (
                        "delta-bad"
                        if yoy_val > 0
        ...
        with st.container(border=False):
            ...
            with k1:
                if kpi_benchmark > 0:
                    diff_text = get_kpi_subtitle(
                        current_val, kpi_benchmark, is_pdi_mode, unit
                    )
                    diff_class = (
                        "delta-bad"
                        if (current_val - kpi_benchmark) > 0
                        else "delta-good"
                        if (current_val - kpi_benchmark) < 0
                        else "delta-neutral"
                    )
            ...
            with k2:
                level_text, level_color = (
                    get_risk_level(current_val, kpi_benchmark)
                    if kpi_benchmark > 0
                    else ("⚪ 基準", "#94a3b8")
                )
                st.markdown(
            ...
            with k3:
                if sub_benchmark > 0:
                    sub_diff_text = get_kpi_subtitle(
                        sub_val, sub_benchmark, sub_is_pdi, sub_unit
                    )
                    sub_diff_class = (
                        "delta-bad"
                        if (sub_val - sub_benchmark) > 0
                        else "delta-good"
                        if (sub_val - sub_benchmark) < 0
                        else "delta-neutral"
                    )
            ...
            with k4:
                if (
                    ...
                    fmt_diff = (
                        f"{diff_abs:+.2f}" if is_pdi_mode else f"{diff_abs:+,.0f}"
                    )
        ...
        with col_t_left:
            st.markdown(
                f'<div class="section-title">📈 歷年趨勢 ({display_area_name})</div>',
                unsafe_allow_html=True,
            )
            with st.container(border=True):
                if not df_area_base.empty:
                    ...
                    trend_agg = (
                        df_area_base.groupby("Year")
                        .agg(val=(agg_col, agg_func))
                        .reset_index()
                        .sort_values("Year")
                    )
                    nat_trend_agg = (
                        df_time_base.groupby("Year")
                        .agg(val=(agg_col, agg_func))
                        .reset_index()
                        .sort_values("Year")
                    )
        ...
        with col_t_right:
            st.markdown(
                f'<div class="section-title">🏅 細部排名 ({display_area_name})</div>',
                unsafe_allow_html=True,
            )
            with st.container(border=True):
                # 長表：一列一個縣市／夜市，欄位為名次與指標值
                entity_metrics = (
                    df_current.groupby(target_entity_col)
                    .agg(val=(agg_col, agg_func))
                    .reset_index()
                ...
                if not entity_metrics.empty:
                    entity_stats = entity_metrics.sort_values(
                    ...
                    entity_stats[mode] = entity_stats["val"].apply(
                        lambda x: fmt.format(x)
                    )

                    df_rank_long = entity_stats[
                        ["排名", target_entity_col, mode]
                    ].rename(columns={target_entity_col: entity_label})
                    st.dataframe(
                        df_rank_long,
        ...
        st.markdown(
            f'<div class="section-title">🎯 {display_area_name} 安全風險象限分析 (維度：{entity_label})</div>',
            unsafe_allow_html=True,
        )
        with st.container(border=True):
            # 象限分析
            ...
            if not df_current.empty:
                ...
                if len(quadrant_stats) > 1:
                    ...
                    fig_scatter.add_hline(
                        ...
                        line_dash="dot",
                        line_color="#94a3b8",
                        annotation_text="PDI 均線",
                    )
                    fig_scatter.add_vline(
                        x=avg_count_line,
                        line_dash="dot",
                        line_color="#94a3b8",
                        annotation_text="件數均線",
                    )
                    ...
                    st.info(
                        f"當前範圍內需有 2 個以上的 {entity_label} 方可繪製分佈象限圖。"
                    )
            else:
                st.info("無足夠資料點繪製象限圖")
        ...
        st.markdown(
            f'<div class="section-title">⚖️ 各 {entity_label} YoY 改善幅度排行總表 <span style="font-size:13px; font-weight:normal; color:#64748b;">{suffix}</span></div>',
            unsafe_allow_html=True,
        )
        st.markdown(
            """
        ...
        with st.container(border=True):
            if sel_year_str == "全部年份":
                ...
                if not df_current.empty and not df_prev.empty:
                    ...
                    prev_stats = (
                        df_prev.groupby(target_entity_col)
                        .agg(val=(agg_col, agg_func))
                        .reset_index()
                    )
                    ...
                    for _, row in curr_stats.iterrows():
                        ...
                        curr_val = row["val"]
                        prev_row = prev_stats[
                            prev_stats[target_entity_col] == entity_name
                        ]
                ...
                if yoy_list:
                    ...
                    if is_pdi_mode:
                        yoy_df["YoY_str"] = yoy_df["YoY"].apply(
                            lambda x: f"+{x:.2f}%" if x > 0 else f"{x:.2f}%"
                        )
                        row_label = "YoY 改善率 (%)"
                    else:
                        yoy_df["YoY_str"] = yoy_df.apply(
                            lambda row: (
                                f"+{row['淨變化']:,.0f} ({row['YoY']:+.2f}%)"
                                if row["淨變化"] > 0
                                else f"{row['淨變化']:,.0f} ({row['YoY']:+.2f}%)"
                            ),
                            axis=1,
                        )
                        row_label = "YoY 淨變化"
                    ...
                    df_yoy_transposed = (
                        yoy_df[["名稱", "排名_str", "YoY_str"]].set_index("名稱").T
                    )
                    ...
                    def color_yoy_row(row):
                        if row.name == row_label:
                            return [
                                ...
                                if "+" in str(v)
                                else "color: #10b981; font-weight: bold;"
                                if "-" in str(v)
                                else ""
                                for v in row
                            ]
                    ...
                    st.dataframe(
                        df_yoy_transposed.style.apply(color_yoy_row, axis=1),
                        use_container_width=True,
                    )
                else:
