...
def normalize_accident_columns(df):
    ...
    if 'accident_datetime' not in df.columns:
        df['accident_datetime'] = pd.to_datetime(df['accident_date'], errors='coerce') + pd.to_timedelta(
...
def get_single_market_redis(lat, lon, radius_km):
    try:
        ...
        logger.error(f"Redis 讀取失敗，cache key: traffic:nearby_v12:{lat:.4f}_{lon:.4f}_3.0_all_sample", exc_info=True)
    return pd.DataFrame()
...
def main():
    ...
    def get_all_nightmarkets():
        ...
        try:
            ...
            # 綁定四層級：
            df['Region'] = df['region'].replace({'東部': '東部與離島', '離島': '東部與離島'})
            df['City'] = df['city']
    ...
    with col_left:
        ...
        if search_mode == "🔍 直接關鍵字搜尋":
            ...
            sel_market = st.selectbox("請輸入或選擇夜市名稱", all_markets, index=all_markets.index(
                def_market) if def_market in all_markets else 0)
        else:
            with st.container(border=True):
                def_region, def_city, def_dist = "北部", "臺北市", "士林區"
                region_opts = sorted(df_market['Region'].dropna().unique())
                sel_region = st.selectbox("區域", region_opts, index=region_opts.index(
                    def_region) if def_region in region_opts else 0)

                city_opts = sorted(df_market[df_market['Region'] == sel_region]['City'].dropna().unique())
                sel_city = st.selectbox("縣市", city_opts, index=city_opts.index(def_city)
                                        if def_city in city_opts else 0)

                dist_opts = sorted(df_market[(df_market['Region'] == sel_region) & (
                    df_market['City'] == sel_city)]['AdminDistrict'].dropna().unique())
                sel_dist = st.selectbox("行政區", dist_opts, index=dist_opts.index(def_dist)
                                        if def_dist in dist_opts else 0)

                m_opts = sorted(df_market[(df_market['Region'] == sel_region) & (df_market['City'] == sel_city) & (
                    df_market['AdminDistrict'] == sel_dist)]['MarketName'].dropna().unique())
                if not m_opts:
                    st.warning("此區域無夜市")
                    st.stop()
    ...
    if sel_w != "全部":
        df_filtered = df_filtered[df_filtered['Weekday'] == {v: k for k, v in week_map.items()}[sel_w]]
    if "白天" in heat_mode:
        df_filtered = df_filtered[(df_filtered['Hour'] >= 6) & (df_filtered['Hour'] < 18)]
    elif "夜間" in heat_mode:
        df_filtered = df_filtered[(df_filtered['Hour'] >= 18) | (df_filtered['Hour'] < 6)]
    ...
    if 'pdi_score' in df_filtered.columns:
        ...
        df_filtered['pdi_score'] = (df_filtered['death_count'] * 10 +
                                    df_filtered['injury_count'] * 2) * df_filtered['weight']
    ...
    with col_right:
        ...
        with title_col:
            st.markdown(
                f"<h4 style='margin-top:0; margin-bottom:10px; color:#1e293b;'>🎯 {sel_market} 自訂周邊 {radius_m} 公尺安全體檢</h4>", unsafe_allow_html=True)
        with count_col:
            st.markdown(
                f"<div style='text-align:right; font-size:15px; color:#64748b; padding-top:6px; white-space:nowrap;'>"
                f"📌 事故總數 <b style='color:#3b82f6; font-size:18px;'>{total_count:,}</b> 件</div>", unsafe_allow_html=True)

        # 這裡下方開始的圖表，皆聯動左側拉桿的 custom 範圍
        chart_h = 240
        X_AXIS_H = 28  # Altair x 軸刻度文字加 tick 佔掉的高度，用來換算成與 plotly 相同的總高
        ...
        with c_radar:
            ...
            st.markdown(
                "<div style='font-size:14px; font-weight:bold; color:#475569; text-align:center;'>🕸️ 環境風險雷達圖</div>", unsafe_allow_html=True)
            risk_df = pd.DataFrame({"risk": ["雨天", "光線不佳", "濕滑路面"], "value": [rain_ratio, dark_ratio, wet_ratio]})
            fig = px.line_polar(risk_df, r="value", theta="risk", line_close=True, markers=True,
            ...
            fig.update_layout(polar=dict(radialaxis=dict(visible=False), angularaxis=dict(tickfont=dict(size=10))), margin=dict(
                l=35, r=35, t=25, b=25), height=chart_h, paper_bgcolor="rgba(0,0,0,0)")
            st.plotly_chart(fig, use_container_width=True)

        with c_cause:
            # Plotly 圓餅圖：Top 5 肇因，其餘併為「其他」，百分比才是佔全部事故的比例
            st.markdown(
                "<div style='font-size:14px; font-weight:bold; color:#475569; text-align:center;'>🔍 Top 5 肇因比例</div>", unsafe_allow_html=True)
            if 'primary_cause' in df_filtered.columns:
                ...
                df_cause['標籤'] = df_cause['肇因'].apply(lambda c: c if len(c) <= 10 else c[:10] + '…')
                fig_c = px.pie(df_cause, names='標籤', values='件數', custom_data=['肇因'],
                               color_discrete_sequence=px.colors.qualitative.Set2)
                fig_c.update_traces(textposition='inside', textinfo='percent', textfont_size=11,
                                    hovertemplate='%{customdata[0]}<br>%{value} 件 (%{percent})<extra></extra>')
                fig_c.update_layout(height=chart_h, margin=dict(l=0, r=0, t=0, b=0),
        ...
        with c_hour:
            st.markdown(
                "<div style='font-size:14px; font-weight:bold; color:#475569; text-align:center;'>🌙 24H 事故熱力</div>", unsafe_allow_html=True)
            ...
            st.altair_chart(
                h_chart.properties(height=chart_h - X_AXIS_H, padding=0).configure_view(strokeWidth=0),
        ...
        m = ui.build_map(False, target_market, layers, d_zoom, radius_m, None,
                         df_for_map, df_market, custom_tiles="OpenStreetMap")
        ...
        if st.session_state.get("ai_report_text"):
            st.markdown(
                f"<div style='margin-top:15px; font-size:13.5px; color:#334155; line-height:1.7;'>{st.session_state.ai_report_text}</div>", unsafe_allow_html=True)
